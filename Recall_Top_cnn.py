...
def process_data_function(sorted_targets, matching_rows, drug_encoding, target_encoding, ff2_modelss, smiles, target):
    ...
    if len(filtered_rows) > 10:
        print(f"Result:")
        print("Result sorce: " + str(matching_rows[0][1][2]) + ", >=sorce_number: " + str(len(filtered_rows)) , str(len(filtered_rows1)))
        ...
        ff2_filtered_rows22_all = [row for row in ff2_sorted_targets_dta_all if row == ff2_matching_rows2_all]
        ff2_indexs  = len(ff2_filtered_rows21_all) + len(filtered_rows)
        print(f"Result:")
        print("Result sorce: " + str(matching_rows[0][1][2]) + ", >=sorce_number: " + str(len(filtered_rows)) , str(len(filtered_rows1)))
        print("DTA_sorce_all:" + str(ff2_matching_rows2_all) + ", top: " + str(ff2_indexs) + ", DTA>=sorce_number: " +  str(len(ff2_filtered_rows21_all)), str(len(ff2_filtered_rows22_all)))     
...
sum = 1061
# 输出 top_all_dict 中的数据
for key, value in top_all_dict.items():
    topk = [0] * 6
    for i in range(0, 6):
        topk[i] = round(value[i] / sum * 100, 2)
